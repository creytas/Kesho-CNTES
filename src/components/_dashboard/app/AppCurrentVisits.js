...
export default function AppCurrentVisits() {
  ...
  useEffect(async () => {
    try {
      const response = await Axios.get(
        `https://kesho-api.herokuapp.com/reporting/annuel`,
        {
          headers: {
      ...
      setMasData(
        await output.rapport_mas_year
          .map((i) => i[0].sereve_nombre)
          .reduce((a, b) => a + b)
      );
      setMamData(
        await output.rapport_mam_year
          .map((i) => i[0].moderee_nombre)
          .reduce((a, b) => a + b)
      );
      console.log(mamData);
      console.log(masData);
      console.log(macData);
      setMacData(
        await output.rapport_mac_year
          .map((i) => i[0].chronique_nombre)
          .reduce((a, b) => a + b)
      );
      setGueris(
        await output.rapport_gueri_year
          .map((i) => i[0].gueri_nombre)
          .reduce((a, b) => a + b)
      );
      setLoader(false);
    } catch (err) {
  ...
  const CHART_DATA = [macData, masData, mamData, gueris];
